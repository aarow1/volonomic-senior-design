...
if C
    ...
    y = eyeCenterL(2,2);
    d = (x + (y - c)*a)/(1 + a^2);
    eyeCenterR(2,1) = 2*d - x;
    eyeCenterR(2,2) = 2*d*a - y + 2*c;
    %     eyeCenterL(2,:) = [boxEyeR(2,1)+2*(abs(boxNose(2,1)+(boxNose(2,3)/2)-boxEyeR(2,1))) boxEyeR(2,2) boxEyeR(2,3:4)];.
    boxEyeR(2,:)=[eyeCenterR(2,1)-boxEyeL(2,3)/2, eyeCenterR(2,2)-boxEyeL(2,4)/2, boxEyeL(2,3:4)];
end
if D
    boxEyeLCart = [boxEyeL(:,1:2), boxEyeL(:,1:2)+boxEyeL(:,3:4)];
    eyeCenterL = [(boxEyeLCart(:,1)+boxEyeLCart(:,3))/2, (boxEyeLCart(:,2)+boxEyeLCart(:,4))/2];

    boxEyeR(1,1:4) = [0 0 0 0];
    a = (mouthCenter(1,2)-noseCenter(1,2))/(mouthCenter(1,1)-noseCenter(1,1));
    c = noseCenter(1,2)-a*noseCenter(1,1);
    x = eyeCenterL(1,1);
    y = eyeCenterL(1,2);
    d = (x + (y - c)*a)/(1 + a^2);
    eyeCenterR(1,1) = 2*d - x;
    eyeCenterR(1,2) = 2*d*a - y + 2*c;
    %     eyeCenterL(2,:) = [boxEyeR(2,1)+2*(abs(boxNose(2,1)+(boxNose(2,3)/2)-boxEyeR(2,1))) boxEyeR(2,2) boxEyeR(2,3:4)];.
    boxEyeR(1,:)=[eyeCenterR(1,1)-boxEyeL(1,3)/2, eyeCenterR(1,2)-boxEyeL(1,4)/2, boxEyeL(1,3:4)];
end
boxEyeLCart = [boxEyeL(:,1:2), boxEyeL(:,1:2)+boxEyeL(:,3:4)];
eyeCenterL = [(boxEyeLCart(:,1)+boxEyeLCart(:,3))/2, (boxEyeLCart(:,2)+boxEyeLCart(:,4))/2];
boxEyeRCart = [boxEyeR(:,1:2), boxEyeR(:,1:2)+boxEyeR(:,3:4)];
