function [cx cy cz] = quintic_spline(nSplines,waypts,time)
%%Quintic fit
...
%     i
    ...
    A(i+nSplines,(i-1)*6+(1:6)) = [1 time(i+1) time(i+1)^2 time(i+1)^3 time(i+1)^4 time(i+1)^5]
    ...
    A(i+3*nSplines,i*6+(1:6)) = [0 0 2 6*time(i+1) 12*time(i+1)^2 20*time(i+1)^3];
