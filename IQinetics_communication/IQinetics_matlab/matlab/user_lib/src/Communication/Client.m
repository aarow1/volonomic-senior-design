    ...
    methods
        ...
        %   obj_id, object identifier with default 0
            ...
            for k = 1:numel(input)
                ...
                s = Wrapper(s);
                long_idn = bitsll(uint32(s.data.type_idn), 8) + uint32(s.data.param_idn);
            ...
            for k = 1:numel(input);
                ...
                if ~obj.outbox.isKey(in.param)
                    long_idn = bitsll(uint32(in.type_idn), 8) + uint32(in.param_idn);
        ...
        % obj.set_verify('speed_kp', 0.01)
            ...
            warning('Function set_verify failed after 10 retries. Value error is %d', val_error);
        end
